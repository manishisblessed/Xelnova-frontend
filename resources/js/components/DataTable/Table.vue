                        ...
                        <tbody :key="tableDataKey">
                            <slot name="body" :show="show">
                                <tr
                                    ...
                                    "
                                    class="sticky top-10 z-[9] bg-gray-100 dark:bg-slate-700 shadow-sm"
                                >
...
const inertiaListener = () => {
    ...
    // Sync filter values from server props to prevent stale filter state
    // This fixes the issue where back/forward navigation shows wrong filter results
    if (queryBuilderProps.value.filters) {
        queryBuilderProps.value.filters.forEach((serverFilter, index) => {
            if (queryBuilderData.value.filters[index]) {
                queryBuilderData.value.filters[index].value = serverFilter.value;
            }
        });
    }
    
    // Sync search inputs as well
    if (queryBuilderProps.value.searchInputs) {
        queryBuilderProps.value.searchInputs.forEach((serverInput, index) => {
            if (queryBuilderData.value.searchInputs[index]) {
                queryBuilderData.value.searchInputs[index].value = serverInput.value;
            }
        });
    }
