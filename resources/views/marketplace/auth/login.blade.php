<x-marketplace.layout>
    ...
    <div class="bg-gray-100 py-12">
        <div class="container mx-auto px-4">
            <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-sm overflow-hidden flex flex-col md:flex-row min-h-[500px]" 
                 x-data="loginForm()">
                <!-- Left Side -->
                <div class="bg-xelnova-green-600 text-white p-8 md:w-2/5 flex flex-col justify-between bg-cover bg-center relative overflow-hidden">
                    <div class="relative z-10">
                        <h2 class="text-3xl font-bold mb-4">Login</h2>
                        <p class="text-lg text-blue-100">Get access to your Orders, Wishlist and Recommendations</p>
                    </div>
                ...
                <div class="p-8 md:w-3/5 flex flex-col justify-center">
                    ...
                    <form x-show="step === 1" @submit.prevent="sendOtp" class="space-y-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Enter Email or Mobile Number</label>
                            <input 
                                type="text" 
                                x-model="identifier"
                        ...
                        <div class="flex items-center">
                            <input 
                                ...
                                id="remember" 
                                x-model="remember"
                                class="w-4 h-4 text-xelnova-green-600 border-gray-300 rounded focus:ring-xelnova-green-500">
                        ...
                        <button type="submit" 
                            ...
                            <span x-text="loading ? 'Sending...' : 'Request OTP'"></span>
                        </button>
                        
                        <div class="text-center">
                            <a href="{{ route('customer.register') }}" class="text-blue-600 font-medium text-sm hover:underline">New to Xelnova? Create an account</a>
                        </div>
                    </form>
                    ...
                    <form x-show="step === 2" @submit.prevent="verifyOtp" class="space-y-6" x-cloak>
                        <div class="text-center mb-4">
                            ...
                            <p x-show="sentChannels.length > 1" class="text-sm text-green-600 mt-1">
                                ✓ OTP sent to both <span x-text="sentChannels.join(' and ')"></span>
                            </p>
                        ...
                        @if(config('app.debug'))
                        <div x-show="debugOtp && Object.keys(debugOtp).length > 0" class="bg-yellow-50 text-yellow-800 p-3 rounded-lg text-sm">
                            <div class="font-bold mb-1">Debug OTPs:</div>
                            <template x-for="(otp, channel) in debugOtp" :key="channel">
                                <div class="flex justify-between">
                                    <span class="capitalize" x-text="channel + ':'"></span>
                                    <span class="font-bold" x-text="otp"></span>
                                </div>
                            </template>
                        </div>
                        @endif
                    ...
                    <form x-show="step === 3" @submit.prevent="completeRegistration" class="space-y-6" x-cloak>
                        <div class="text-center mb-4">
                            ...
                            <h3 class="text-xl font-bold text-gray-800">Welcome to Xelnova!</h3>
                            <p class="text-gray-600">Please enter your name to complete registration</p>
                        </div>
                        ...
                        <button type="submit" 
                            ...
                            <span x-text="loading ? 'Setting up...' : 'Continue'"></span>
                        </button>
                    </form>
                </div>
            </div>
    ...
    <script>
        function loginForm() {
            return {
                ...
                resendTimer: 0,
                debugOtp: null,
                sentChannels: [], // Track which channels received OTP
                ...
                async sendOtp() {
                    ...
                    try {
                        ...
                        if (data.success) {
                            this.step = 2;
                            this.sentChannels = data.channels || [this.identifierType];
                            this.startResendTimer(data.expires_in || 600);
